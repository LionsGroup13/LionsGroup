...
(function(ah) {
    ...
    D.cross = function(j) {
        var i = this.y * j.z - this.z * j.y,
            az = this.z * j.x - this.x * j.z,
            ay = this.x * j.y - this.y * j.x;
    ...
    function ae() {
        ...
        az = j.getContext("2d");
        az.strokeStyle = "#ff0000";
        az.shadowColor = "#ff0000";
        az.shadowBlur = 3;
        az.globalAlpha = 0;
    ...
    function T(aH, aA, j, aJ, aB, aG, aC, az, aF, aI) {
        var aE = aB + (aC || 0) + (az && az[0] < 0 ? H(az[0]) : 0),
            ay = aB + (aC || 0) + (az && az[1] < 0 ? H(az[1]) : 0),
            aD, aK;
        aH.font = aA;
        aH.textBaseline = "top";
        aH.fillStyle = j;
    ...
    function t(aC, aK, aE, aP, aF, aM, aD, aN, aL, aJ, i, aH, aB, aI, aR) {
        var az = aE + H(i[0]) + 2 * (aJ + aH) + aN,
            aG = aP + H(i[1]) + 2 * (aJ + aH) + aN,
            aA, aS, aQ, aO, ay, j;
        aA = L(az, aG);
        if (!aA) {
            return null
    ...
    function ar(aI, ay, aH, aC, aN, aJ, aD) {
        var aE = aI.width + (2 * aN) + aH,
            j = aI.height + (2 * aN) + aH,
            aG = L(aE, j),
            aK, aA, aM, az, aL, aF, aB;
        if (!aG) {
            return null
        }
        aA = aM = aH / 2;
    ...
    function V(aD, aG, aH, az) {
        var aI = H(az[0]),
            aE = H(az[1]),
            aA = aD.width + (aI > aH ? aI + aH : aH * 2),
            j = aD.height + (aE > aH ? aE + aH : aH * 2),
            aC = (aH || 0) + (az[0] < 0 ? aI : 0),
            ay = (aH || 0) + (az[1] < 0 ? aE : 0),
            aB, aF;
        aB = L(aA, j);
    ...
    function r(aK, aC, aI) {
        var aJ = parseInt(aK.toString().length * aI),
            aB = parseInt(aI * 2 * aK.length),
            az = L(aJ, aB),
            aF, j, aA, aE, aH, aG, ay, aD;
        if (!az) {
            return null
        }
        aF = az.getContext("2d");
    ...
    function U(az, ay) {
        var j = A.defaultView,
            i = ay.replace(/\-([a-z])/g, function(aA) {
    ...
    function O(aA, aB) {
        var az, ay, i = parseInt(U(aB, "width")) / aB.width,
            j = parseInt(U(aB, "height")) / aB.height;
        if (ac(aA.offsetX)) {
            az = {
                x: aA.offsetX,
                y: aA.offsetY
    ...
    function z(ay) {
        var j = ay.target || ay.fromElement.parentNode,
            i = w.tc[j.id];
        if (i) {
            i.mx = i.my = -1;
            i.UnFreeze();
    ...
    function W(ay) {
        var aB = A.getElementById(ay),
            i = aB.getBoundingClientRect(),
            aE = A.documentElement,
            aC = A.body,
            aD = window,
            az = aD.pageXOffset || aE.scrollLeft,
            aF = aD.pageYOffset || aE.scrollTop,
            aA = aE.clientLeft || aC.clientLeft,
            j = aE.clientTop || aC.clientTop;
        return {
            x: i.left + az - aA,
            y: i.top + aF - j
        }
    ...
    v.DrawColourImage = function(aD, aG, aE, aH, aC, i, aK, j, aF) {
        var aI = aD.canvas,
            aA = ~~q(aG, 0),
            az = ~~q(aE, 0),
            aB = au(aI.width - aA, aH) + 0.5 | 0,
            aJ = au(aI.height - az, aC) + 0.5 | 0,
            ay;
        if (n) {
    ...
    b.Measure = function(aC, j) {
        ...
        if (j.txtOpt) {
            var az = j.txtScale,
                aA = az * this.textHeight,
                aB = aA + "px " + this.textFont,
                ay = [az * j.shadowOffset[0], az * j.shadowOffset[1]],
                i;
            aC.font = aB;
    ...
    b.DrawImageIE = function(aA, aE, az) {
        var j = this.image,
            aF = this.sc,
            aD = j.width = this.w * aF,
            ay = j.height = this.h * aF,
            aC = (this.x * aF) + aE - (aD / 2),
            aB = (this.y * aF) + az - (ay / 2);
        aA.setTransform(1, 0, 0, 1, 0, 0);
        aA.globalAlpha = this.alpha;
    ...
    function w(aD, j, az) {
        var ay, aB, aC = A.getElementById(aD),
            aA = ["id", "class", "innerHTML"];
        if (!aC) {
            throw 0
        }
    ...
    M.CreateTag = function(aB, j) {
        var aE = aB.getElementsByTagName("img"),
            aA, aF, aD, ay, aC, az;
        j = j || [0, 0, 0];
        if (aE.length) {
            aA = new Image;
    ...
    M.UpdateTag = function(ay, i) {
        var az = this.textColour || U(i, "color"),
            j = this.textFont || m(U(i, "font-family"));
        ay.a = i;
        ay.title = i.title;
    ...
    M.Draw = function(aI) {
        ...
        var aC = this.canvas,
            ...
            aH = aC.height,
            aK = 0,
            az = (aI - this.time) * this.interval / 1000,
            aG = aA / 2 + this.offsetX,
            aF = aH / 2 + this.offsetY,
            aO = this.ctxt,
            aE, aP, aM, ay = -1,
            aB = this.taglist,
            aL = aB.length,
            j = this.frontSelect,
            aJ = (this.centreFunc == aq),
            aD;
        this.time = aI;
    ...
    M.Transform = function(aB, i, aD) {
        if (i || aD) {
            var j = ab(i),
                ...
                aE = ab(aD),
                aA = u(aD),
                ay = new N([aA, 0, aE, 0, 1, 0, -aE, 0, aA]),
                az = new N([1, 0, 0, 0, aC, -j, 0, j, aC]);
    ...
    M.AnimateDrag = function(j, aA, az) {
        var i = this,
            ay = 100 * az * i.maxSpeed / i.max_radius / i.zoom;
        if (i.dx || i.dy) {
    ...
    M.Drag = function(aA, az) {
        if (this.dragControl && this.down) {
            var ay = this.dragThreshold * this.dragThreshold,
                j = az.x - this.down.x,
                i = az.y - this.down.y;
