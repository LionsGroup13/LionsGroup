...
// html localization
gulp.task("ruHtml:build", function () {
  return gulp
    .src(path.src.ruHtml)
    .pipe(plumber())
    .pipe(rigger())
    .pipe(gulp.dest(path.build.ruHtml))
    .pipe(webserver.reload({ stream: true }));
});
...
gulp.task("watch", function () {
  ...
  gulp.watch(path.watch.html, gulp.series("ruHtml:build"));
...
gulp.task(
  "default",
  gulp.series("build", "size", gulp.parallel("webserver", "watch"))
);

// production
gulp.task("prod", gulp.series("build", "size"));
